    ...
    <a href="{{route('customers.index')}}" class="transition duration-300 ease-in-out hover:-translate-y-2 hover:shadow-xl">
        <div class="bg-white rounded-xl p-5 flex items-center gap-4 shadow-sm">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900">{{ $myCustomers }}</p>
            </div>
        </div>
    </a>
    <a href="{{route('leads.index')}}" class="transition duration-300 ease-in-out hover:-translate-y-2 hover:shadow-xl">
        <div class="bg-white rounded-xl p-5 flex items-center gap-4 shadow-sm">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900">{{ $myActiveLeads }}</p>
            </div>
        </div>
    </a>
    <a href="{{route('follow-ups.index')}}" class="transition duration-300 ease-in-out hover:-translate-y-2 hover:shadow-xl">
        <div class="bg-white rounded-xl p-5 flex items-center gap-4 shadow-sm">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900">{{ $myPendingFollowUps }}</p>
            </div>
        </div>
    </a>
    <a href="{{route('follow-ups.index')}}" class="transition duration-300 ease-in-out hover:-translate-y-2 hover:shadow-xl">
        <div class="bg-white rounded-xl p-5 flex items-center gap-4 shadow-sm">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900">{{ $myCompletedFollowUps }}</p>
            </div>
        </div>
    </a>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
    ...
    <div class="bg-white rounded-xl shadow-sm">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h3 class="font-semibold text-gray-900 text-sm">My Leads by Status</h3>
            <a href="{{ route('leads.index') }}" class="text-xs text-blue-600 hover:underline">View all</a>
        </div>
        <ul class="divide-y divide-gray-50">
            @forelse($myLeadsByStatus as $status => $count)
    ...
    <div class="lg:col-span-2 bg-white rounded-xl shadow-sm">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h3 class="font-semibold text-gray-900 text-sm">Upcoming Follow-ups</h3>
            <a href="{{ route('follow-ups.create') }}" class="text-xs text-blue-600 hover:underline">+ New</a>
        </div>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($upcomingFollowUps as $followUp)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-5 py-3 {{ $followUp->isOverdue() ? 'text-green-600 font-semibold' : 'text-gray-600' }}">
                            ...
                            <td class="px-5 py-3">
                                <form method="POST" action="{{ route('follow-ups.complete', $followUp) }}">
                                    ...
                                    @method('PATCH')
                                    <button type="submit"
                                            class="px-3 py-1 text-xs font-medium text-green-700 bg-green-50 hover:bg-green-100 rounded-lg transition-colors">
                                        Done
                                    </button>
                                </form>
                            </td>
...
<div class="bg-white rounded-xl shadow-sm">
    <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        <h3 class="font-semibold text-gray-900 text-sm">My Recent Activities</h3>
        <a href="{{ route('activities.create') }}" class="text-xs text-blue-600 hover:underline">+ Log Activity</a>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            <thead>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-5 py-3">
                            @if($activity->customer)
                                <a href="{{ route('customers.show', $activity->customer) }}" class="text-blue-600 hover:underline">{{ $activity->customer->full_name }}</a>
                            @elseif($activity->lead)
                                <a href="{{ route('leads.show', $activity->lead) }}" class="text-blue-600 hover:underline">{{ $activity->lead->name }}</a>
                            @else
                                <span class="text-gray-400">—</span>
                            @endif
                        </td>
